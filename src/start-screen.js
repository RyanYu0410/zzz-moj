...
function initStartScreen(start){
 ...
 document.body.append(screen);let selected=0;
 ...
 AGENTS.forEach(([name,art],id)=>{const b=document.createElement('button');b.className='agent-option';b.innerHTML='<img src="winners/'+art+'.png" alt=""><span>'+name+'</span>';b.onclick=()=>select(id);screen.querySelector('.agent-picker').append(b)});
 screen.addEventListener('cancel',e=>e.preventDefault());screen.querySelector('#start-play').onclick=()=>{try{localStorage.setItem('riichi-character',selected)}catch{}screen.close();start([selected,...AGENTS.map((_,i)=>i).filter(i=>i!==selected)].map(i=>AGENTS[i]));};
