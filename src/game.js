...
let match,human,decision=null,reply=null,selected=-1,riichiPick=false,kanPick=false,callFilter=null,lastText='正在发牌…',sound=false,ctx,actionEffects=[],effectsTimers=[],resultOpen=false;
...
function selectDiscard(index){if(decision?.type!=='turn')return;const p=discardCode(index),allowed=riichiPick?decision.riichi:decision.discards;if(!allowed.includes(p))return;if(selected===index)discard();else{selected=index;tone(540);render()}}
...
function render(){if(!match?.model.shan)return;const model=match.model,seat=ownSeat(),hand=model.shoupai[seat];
 ...
 $('discard').disabled=decision?.type!=='turn'||selected<0;$('win').disabled=decision?.type!=='turn'||!decision.win;$('riichi').disabled=decision?.type!=='turn'||!decision.riichi.length;$('riichi').textContent=hand.lizhi?'已立直':riichiPick?'取消立直':'立直';for(const kind of ['chi','pon','kan']){const available=decision?.type==='response'?decision.calls.some(m=>meldKind(m)===kind):kind==='kan'&&decision?.type==='turn'&&decision.kan.length>0;$(kind).disabled=!available;$(kind).setAttribute('aria-expanded',kind==='kan'&&decision?.type==='turn'?kanPick:callFilter===kind);$(kind).title=available?'选择'+({chi:'吃',pon:'碰',kan:'杠'}[kind])+'牌组合':({chi:'上家弃牌可组成顺子时可吃',pon:'对手弃牌与你的对子相同时可碰',kan:'持有四张同牌或可加杠时开放'}[kind])}$('abort').hidden=decision?.type!=='turn'||!decision.abort;
 $('status').textContent=decision?.type==='response'?(decision.rob?'抢杠机会':'可以鸣牌／荣和'):decision?.type==='turn'?(riichiPick?'立直 · 选择切牌':hand.lizhi?'已立直 · 摸切／自摸':'轮到你出牌'):lastText;
...
$('discard').onclick=discard;$('win').onclick=()=>{if(decision?.type==='turn'&&decision.win)submit({hule:'-'})};$('riichi').onclick=()=>{if(decision?.type==='turn'&&decision.riichi.length){riichiPick=!riichiPick;selected=-1;kanPick=false;render()}};$('chi').onclick=()=>chooseCallKind('chi');$('pon').onclick=()=>chooseCallKind('pon');$('kan').onclick=()=>chooseCallKind('kan');$('abort').onclick=()=>{if(decision?.abort)submit({daopai:'-'})};$('call-pass').onclick=()=>{if(decision?.type==='response')submit({})};
...
document.addEventListener('keydown',e=>{if($('modal').open||$('victory').open||document.querySelector('[popover]:popover-open'))return;if(decision?.type==='response'){if(e.key==='Escape')submit({});return}if(decision?.type!=='turn')return;if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();const n=handTiles(match.model.shoupai[ownSeat()]).length,step=e.key==='ArrowRight'?1:-1;for(let k=0;k<n;k++){selected=(selected+step+n)%n;if((riichiPick?decision.riichi:decision.discards).includes(discardCode(selected)))break}render()}if(e.key==='Enter'&&document.activeElement.tagName!=='BUTTON')discard()});
