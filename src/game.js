...
function showResult(options){resultOpen=true;$('modal').classList.remove('winner-result');const result=options.result,model=match.model;
 if(options.type==='match'){const saved=preferences.saveMatch(matchId,result,matchSettings);show('<h2>对局结束</h2><p>'+ (saved?'已保存到对局历史':'历史保存失败，请下载牌谱。') +'</p><div class="settlement">'+result.rank.map((rank,id)=>({rank,id})).sort((a,b)=>a.rank-b.rank).map(({rank,id})=>'<p><b>#'+rank+' '+CHARACTERS[id]+'</b><span>'+result.defen[id].toLocaleString()+' 点</span></p>').join('')+'</div><button id="next" class="primary">再开一场</button><button id="download-log">保存牌谱</button>');$('next').onclick=newGame;$('download-log').onclick=()=>downloadLog(result);return}
...
document.addEventListener('keydown',e=>{if($('modal').open||$('victory').open||document.querySelector('[popover]:popover-open, #preferences-panel[open]'))return;if(decision?.type==='response'){if(e.key==='Escape')submit({});return}if(decision?.type!=='turn')return;if(e.key==='Escape'&&!$('call-pass').hidden){$('call-pass').click();return}if(e.key==='ArrowLeft'||e.key==='ArrowRight'){e.preventDefault();const n=handTiles(match.model.shoupai[ownSeat()]).length,step=e.key==='ArrowRight'?1:-1;for(let k=0;k<n;k++){selected=(selected+step+n)%n;if((riichiPick?decision.riichi:decision.discards).includes(discardCode(selected)))break}render()}if(e.key==='Enter'&&document.activeElement.tagName!=='BUTTON')discard()});
