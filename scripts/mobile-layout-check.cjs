...
for(const [w,h] of [[390,844],[400,600],[700,700],[844,390],[1440,900]]){
 ...
 for(const selector of ['#hand','#round-toggle','#scores-toggle','#menu-toggle']){const b=await page.locator(selector).boundingBox();assert(b&&b.x>=-1&&b.y>=0&&b.x+b.width<=w+1&&b.y+b.height<=h+1,selector+' outside '+w+'x'+h)}
 for(const selector of ['#round-toggle','#scores-toggle','#menu-toggle']){const b=await page.locator(selector).boundingBox();assert(b.width>=43.9&&b.height>=43.9,selector+' touch target too small')}
