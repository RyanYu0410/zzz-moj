...
(async()=>{
 ...
 if(!await page.locator('#game-menu').isVisible())await page.locator('#menu-toggle').click();await page.locator('#new').click();await page.locator('#reset').click();assert(await page.locator('#start-screen').isVisible());await page.locator('#start-play').click();await page.waitForFunction(()=>mahjongTest.decision);assert.equal(await page.locator('#melds .meld').count(),0);
