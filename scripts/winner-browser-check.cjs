...
(async()=>{const b=await chromium.launch({channel:'chrome',headless:true}),p=await b.newPage({viewport:{width:1280,height:960}}),errors=[];p.on('pageerror',e=>errors.push(e.message));await p.goto('http://localhost:8080/?test');await p.waitForFunction(()=>mahjongTest.decision);
 ...
 for(let id=0;id<4;id++){
  ...
  if(id===1)await p.waitForTimeout(1000);if(id===1)await p.screenshot({path:'/tmp/zzz-billy-reveal.png'});
  ...
  assert(await p.locator('#modal').evaluate(e=>{const b=e.getBoundingClientRect();return b.width===innerWidth&&b.height===innerHeight&&getComputedStyle(e).borderTopWidth==='0px'}));assert.equal(await p.locator('#modal .win-spark').count(),18);if(id===2)await p.waitForTimeout(1000);if(id===2)await p.screenshot({path:'/tmp/zzz-miyabi-results.png'});
  await p.evaluate(()=>document.getElementById('modal').close());
 }
 await p.setViewportSize({width:390,height:844});
 await p.evaluate(()=>{const a=mahjongTest;a.showVictory({type:'result',result:{l:0,baojia:1,shoupai:'m123p456s789z11555',fu:40,fanshu:2,defen:2600,hupai:[{name:'役牌 白',fanshu:1}],fenpei:[2600,-2600,0,0]}})});await p.locator('#reveal-result').click();await p.waitForTimeout(1000);await p.screenshot({path:'/tmp/zzz-winner-mobile.png'});assert(await p.evaluate(()=>{const d=document.getElementById('modal');return d.scrollWidth<=d.clientWidth+2}));await p.locator('#next').scrollIntoViewIfNeeded();assert(await p.locator('#next').isVisible());await p.emulateMedia({reducedMotion:'reduce'});assert.equal(await p.locator('#modal .win-streak').evaluate(e=>getComputedStyle(e).display),'none');assert.deepEqual(errors,[]);await b.close();console.log('PASS: table racks, hidden opponents, four distinct win portraits, click-to-reveal settlement, mobile result layout')})().catch(e=>{console.error(e);process.exit(1)});
